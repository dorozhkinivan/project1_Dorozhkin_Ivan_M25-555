"""Модуль содержит неизменяемые данные игры: карту комнат и список команд."""

# Структура puzzle: (Текст вопроса, [список_правильных_ответов])
ROOMS = {
    'entrance': {
    ...
    'hall': {
        ...
        'puzzle': ('На пьедестале надпись: "Назовите число, которое идет после '
                   'девяти". Введите ответ цифрой или словом.', ['10', 'десять'])
    },
    'trap_room': {
        ...
        'exits': {'west': 'entrance'},
        'items': ['rusty_key'],
        'puzzle': ('Система плит активна. Чтобы пройти, назовите '
                   'слово "шаг" три раза подряд (введите "шаг шаг шаг")',
                   ['шаг шаг шаг'])
    },
    'library': {
    ...
    'secret_chamber': {
        'description': 'Тайная камера, скрытая за '
                       'книжной полкой в библиотеке. В центре — старинный алтарь.',
        'exits': {'south': 'library'},
        'items': ['mysterious_amulet'],
        'puzzle': (
            'На алтаре надпись: "Скажи имя того, кто хранит '
            'тайны веков" (ответ — одно слово, имя мифологического персонажа)',
            ['Гермес', 'гермес']
        )
    },
    'dark_cellar': {
        'description': 'Тёмный подвал, слышен шум воды. В углу стоит ржавая решётка.',
        'exits': {'up': 'armory'},
        'items': ['old_lantern'],
        'puzzle': (
            'Решётка заперта на замок с шифром. На ней выцарапано: "Число, '
            'равное сумме первых четырёх натуральных чисел" (введите ответ цифрой)',
            ['10']
        )
    }
}
